...
//! two OpenAI-compatible plugin types (chat + embedding) onto the sync
//! FFI trait the cdylib vtable expects ([`SyncBackendPlugin`]). Each
//! wrapper owns a private multi-thread runtime and `block_on`s the
//! async logic; the make-time [`HostHandle`] is turned into an
//! `Arc<dyn BackendHost>` (via [`HostHandleBackendHost`]) that is
//! passed to `register_profile`.
//!
...
//! - Neither compat type exposes `set_host_handle`, so the optional
//!   macro arm is omitted for both; the host-handle is still wrapped
...
use crate::embedding_plugin::CompatEmbeddingPlugin;
use crate::plugin::CompatChatPlugin;
...
fn build_bridge_runtime(thread_name: &str) -> tokio::runtime::Runtime {
    tokio::runtime::Builder::new_multi_thread()
        ...
        .enable_all()
        .build()
        .unwrap_or_else(|e| panic!("compat cdylib: tokio runtime init failed: {e}"))
}
...
// --- chat (no `set_host_handle`) -------------------------------------
backend_cdylib_wrapper! {
    /// `SyncBackendPlugin` bridge over [`CompatChatPlugin`].
    CompatChatCdylib, CompatChatPlugin, "mcpg-llm-compat-chat"
}

// --- embedding -------------------------------------------------------
backend_cdylib_wrapper! {
    /// `SyncBackendPlugin` bridge over [`CompatEmbeddingPlugin`].
    CompatEmbeddingCdylib, CompatEmbeddingPlugin, "mcpg-llm-compat-embedding"
}

// cdylib export — both entities under `dev.mcpg.backend.llm.compat`.
// Each entity reports its real kind (compat.chat, compat.embedding)
...
mcpg_plugin_sdk::declare_plugin! {
    ...
    entities: [
        backend as chat {
            inner_name: "",
            plugin_type: CompatChatCdylib,
            factory: |cfg, host: ::mcpg_plugin_sdk::HostHandle|
                CompatChatCdylib::from_host_config(cfg, host),
        },
        backend as embedding {
            inner_name: "embedding",
            plugin_type: CompatEmbeddingCdylib,
            factory: |cfg, host: ::mcpg_plugin_sdk::HostHandle|
                CompatEmbeddingCdylib::from_host_config(cfg, host),
        },
    ],
}
